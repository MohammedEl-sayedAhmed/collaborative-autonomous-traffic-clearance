...
def build_split_env(cfg: ScenarioConfig, n_envs: int, seed: int):
    ...
    features = 12 + 2 * cfg.num_lanes + 4 * cfg.num_neighbors
    return make_agent_split(venv, cfg.num_cooperators, features)
...
def train(cfg: ScenarioConfig, timesteps: int, n_envs: int, seed: int, label: str,
    ...
    status = "failed"
    try:
        vec = build_split_env(cfg, n_envs, seed)
        writer = RunWriter(runs_dir, label, cfg, mode="train") if log else None
        if writer is not None:
            writer.meta["config"]["algo"] = "ippo-shared"
...
def main(argv=None):
    ...
    label = a.label or f"ippo-{a.preset}"
